...
if [ -f "$2" ]; then
	...
	-vaapi_device $1 \
	-n -i "$2" \
	-vf 'format=nv12|vaapi,hwupload,scale_vaapi=w=1920:h=-2:format=nv12' \
	-c:v hevc_vaapi \
	-profile:v main -level:v 5.1 \
	-c:a libfdk_aac -c:s copy "1/$2"
fi
}
...
declare -a files=(*.mp4)
for (( i=0; i<${#files[*]}; i=i+4 ))
do
  encode_vaapi /dev/dri/renderD129 "${files[$i]}" & \
  encode_vaapi /dev/dri/renderD128 "${files[$i+1]}" & \
  encode_vaapi /dev/dri/renderD129 "${files[$i+2]}" & \
  encode_vaapi /dev/dri/renderD128 "${files[$i+3]}" && fg
done
