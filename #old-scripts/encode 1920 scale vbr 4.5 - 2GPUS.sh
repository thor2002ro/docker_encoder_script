...
encode_vaapi () {
    if [ -f "$2" ]; then
        ...
        docker run --rm --device /dev/dri -v "$PWD":/media -w /media --network none --name ffmpeg_${gpu_number}_${container_count} ffmpeg-vaapi -stats \
            ...
            -vf 'format=nv12|vaapi,hwupload,scale_vaapi=w=1920:h=-16:format=nv12' \
            -c:v hevc_vaapi -rc_mode VBR -b:v 4.5M -maxrate 9M -strict unofficial \
            -profile:v main -level:v 5.1 \
            -threads $num_cores \
            -c:a aac -c:s copy -map 0:v -map 0:a -map 0:s "1/$filename" > "$log_file" 2>&1 &
    fi
}
