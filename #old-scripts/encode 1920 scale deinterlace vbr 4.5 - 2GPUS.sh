...
encode_vaapi () {
    if [ -f "$2" ]; then
        ...
        docker run --rm --device /dev/dri -v "$PWD":/media -w /media --network none --name ffmpeg_${gpu_number}_${container_count} ffmpeg-vaapi -stats \
            ...
            -vf 'format=nv12|vaapi,hwupload,scale_vaapi=w=1920:h=-16:format=nv12,deinterlace_vaapi=rate=field:auto=1' \
            -c:v hevc_vaapi -rc_mode VBR -b 4.5M -maxrate 9M -strict unofficial \
            -profile:v main -level:v 5.1 \
            -threads $num_cores \
            -c:a aac -c:s copy -map 0:v -map 0:a "1/$filename" > "$log_file" 2>&1 &
    fi
}
...
find_video_files() {
    local input_dir="$1"
    find "$input_dir" -type f \( -name "*.mp4" -o -name "*.mkv" \)
}

# Main Encoding Loop:
# Updated to use the find_video_files function to retrieve the list of video files
declare -a files=($(find_video_files "$input_dir"))
total_files=${#files[*]}
i=0
...
#delete file after encodeing by docker